...
build_slice() {
  ...
  if [ "$skip_build" -eq 0 ]; then
    ...
    cmake_args=(
      ...
      -DTELEGRAM_PPC_VERSION="${TELEGRAM_PPC_VERSION:-0.2.1}" \
...
normalize_bundle() {
...

  {
    ...
    "$TELEGRAM_PPC_OTOOL" -L "$target" | awk '/^[[:space:]]/ {print $1}' | while read -r load; do
      ...
      [ -e "$fw/$name" ] || continue
      case "$load" in
        @executable_path/*|/System/*) continue ;;
      esac
...
verify_relocatable_bundle() {
  ...
  local bad_loads bad_bundled_loads
  ...
  bad_bundled_loads="$(
    {
      ...
      "$TELEGRAM_PPC_OTOOL" -L "$target" 2>/dev/null | awk '/^[[:space:]]/ {print $1}' | while read -r load; do
        [ -n "$load" ] || continue
        name="$(basename "$load")"
        [ -e "$app/Contents/Frameworks/$name" ] || continue
        case "$load" in
  ...
  if [ -n "$bad_bundled_loads" ]; then
    echo "bundle references bundled libraries through non-bundle load paths:" >&2
    echo "$bad_bundled_loads" >&2
    exit 1
  fi

  verify_ppc_tdlib_patch "$app"
}

verify_ppc_tdlib_patch() {
  local app=$1
  local tdlib="$app/Contents/Frameworks/libtdjson.dylib"
  local tmpdir slice
